...
class Administracion_inmueblesController extends Administracion_mainController
{
	...
	public function getData()
	{
		...
		if ($this->_getSanitizedParam("inmueble_deposito") == '') {
			$data['inmueble_deposito'] = '0';
		} else {
			$data['inmueble_deposito'] = $this->_getSanitizedParam("inmueble_deposito");
		}
		if ($this->_getSanitizedParam("inmueble_importante") == '') {
			$data['inmueble_importante'] = '0';
		} else {
			$data['inmueble_importante'] = $this->_getSanitizedParam("inmueble_importante");
		}
	...
	public function validarinmuebleAction()
	{

		$this->setLayout('blanco');
		$ref = $this->_getSanitizedParam("ref");
		$data = $this->mainModel->getList(" ref = '$ref'");
		if ($data) {
			echo json_encode(array('status' => 'error', 'message' => 'La referencia ya se encuentra registrada'));
		} else {
			echo json_encode(array('status' => 'success', 'message' => 'Referencia válida'));
		}
	}
	...
	protected function filters()
	{
		if ($this->getRequest()->isPost() == true) {
			...
			$parramsfilter['ndepiso'] =  $this->_getSanitizedParam("ndepiso");
