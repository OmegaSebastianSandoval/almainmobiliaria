			...
			<div class="row">
				...
				<div class="col-3">
					...
					<label class="input-group">
						...
						<input type="text" class="form-control" name="ref" value="<?php echo $this->getObjectVariable($this->filters, 'ref') ?>"></input>
					</label>
				</div>
				<div class="col-3">
					...
					<label class="input-group">
						...
						<select class="form-control" name="departamento">
							...
							<?php foreach ($this->list_departamento as $key => $value) : ?>
								<option value="<?= $key; ?>" <?php if ($this->getObjectVariable($this->filters, 'departamento') ==  $key) {
																	echo "selected";
																} ?>><?= $value; ?></option>
							<?php endforeach ?>
						</select>
					...
					<label class="input-group">
						...
						<select class="form-control" name="ciudad">
							...
							<?php foreach ($this->list_ciudad as $key => $value) : ?>
								<option value="<?= $key; ?>" <?php if ($this->getObjectVariable($this->filters, 'ciudad') ==  $key) {
																	echo "selected";
																} ?>><?= $value; ?></option>
							<?php endforeach ?>
						</select>
					...
					<label class="input-group">
						...
						<select class="form-control" name="sector">
							...
							<?php foreach ($this->list_sector as $key => $value) : ?>
								<option value="<?= $key; ?>" <?php if ($this->getObjectVariable($this->filters, 'sector') ==  $key) {
																	echo "selected";
																} ?>><?= $value; ?></option>
							<?php endforeach ?>
						</select>
					...
					<label class="input-group">
						...
						<select class="form-control" name="localidad">
							...
							<?php foreach ($this->list_localidad as $key => $value) : ?>
								<option value="<?= $key; ?>" <?php if ($this->getObjectVariable($this->filters, 'localidad') ==  $key) {
																	echo "selected";
																} ?>><?= $value; ?></option>
							<?php endforeach ?>
						</select>
				...
				<div class="col-3">
					...
					<label class="input-group">
						...
						<input type="text" class="form-control" name="estrato" value="<?php echo $this->getObjectVariable($this->filters, 'estrato') ?>"></input>
					</label>
				</div>
				<div class="col-3">
					...
					<label class="input-group">
						...
						<input type="text" class="form-control" name="tiempoconstruido" value="<?php echo $this->getObjectVariable($this->filters, 'tiempoconstruido') ?>"></input>
					</label>
				</div>
				<div class="col-3">
					...
					<label class="input-group">
						...
						<input type="text" class="form-control" name="ndepiso" value="<?php echo $this->getObjectVariable($this->filters, 'ndepiso') ?>"></input>
					</label>
				</div>
				<div class="col-3">
					<label>&nbsp;</label>
			...
			<table class=" table table-striped  table-hover table-administrator text-left">
				<thead>
					<tr>
						...
						<td>t&iacute;tulo</td>
						<!--<td>estrato</td>-->
						<!--<td>tiempo construido</td>-->
						<!--<td>n&uacute;mero de piso</td>-->
						<td width="200"></td>
					</tr>
				</thead>
				<tbody>
					<?php foreach ($this->lists as $content) { ?>
						...
						<tr>
							...
							<td><?= $content->ref; ?></td>
							<td><?= $content->area; ?></td>
							<td><?= $this->list_departamento[$content->departamento]; ?>
							<td><?= $this->list_ciudad[$content->ciudad]; ?>
							<td><?= $this->list_sector[$content->sector]; ?>
							<td><?= $this->list_localidad[$content->localidad]; ?>
							<td><?= $content->titulo; ?></td>
							<!--<td><?= $content->estrato; ?></td>-->
							<!--<td><?= $content->tiempoconstruido; ?></td>-->
							<!--<td><?= $content->ndepiso; ?></td>-->
							<td class="text-right">
								<div>
									<a class="btn btn-verde btn-sm" href="/administracion/fotos/?inmueble=<?= $id ?>" data-bs-toggle="tooltip" data-placement="top" title="Agregar Imagen"><i class="fa-solid fa-image"></i></a>

									<a class="btn btn-rosado btn-sm" href="/administracion/fotos/cargamasiva?inmueble=<?= $id ?>" data-bs-toggle="tooltip" data-placement="top" title="Carga masiva Imagenes"><i class="fas fa-images"></i></a>
